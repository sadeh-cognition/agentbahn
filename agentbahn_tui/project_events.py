...
def fetch_project_events(
    project_id: int,
    *,
    fetch_features_command: Callable[[int | None], FeatureListResponse] = fetch_features,
    fetch_tasks_command: Callable[[int], TaskListResponse] = fetch_tasks,
    fetch_event_logs_command: Callable[[str, int], EventLogListResponse] = fetch_event_logs,
) -> EventLogListResponse:
    related_entity_ids: list[tuple[str, int]] = [(PROJECT_EVENT_ENTITY_TYPE, project_id)]
    related_entity_ids.extend(
        (FEATURE_EVENT_ENTITY_TYPE, feature.id) for feature in fetch_features_command(project_id)
    )
    related_entity_ids.extend(
    ...
    return sorted(deduplicated_events.values(), key=lambda event: event.id, reverse=True)
